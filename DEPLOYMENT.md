...
### Шаг 3: Получение домена Railway

1. После создания проекта откройте Settings → Networking
2. Нажмите "Generate Domain"
3. Скопируйте домен (например: `your-app-production.up.railway.app`)
...
MONTHLY_LIMIT=10
WEBHOOK_DOMAIN=your-app-production.up.railway.app
```

**ВАЖНО:**
- Переменная `PORT` не нужна - Railway установит её автоматически
- `WEBHOOK_DOMAIN` должен быть БЕЗ `https://` (только домен!)
- Webhook режим намного надёжнее polling для production!
...
### Шаг 6: Настройка Restart Policy
...
# Должен вернуть:
# {"status":"ok","bot":"running","mode":"webhook","uptime":123.45,"timestamp":"2024-..."}
```

**Важно:** Проверьте, что `"mode":"webhook"` - это означает, что бот работает в надёжном режиме.
...
- ✅ Не засыпает, мгновенный ответ
- ✅ Меньше нагрузка на сервер
- ✅ Надёжнее для Railway/Render
- ⚠️ Требует публичный домен (есть в Railway)

**Polling (для локальной разработки):**
- ✅ Проще для локального тестирования
- ❌ Может обрываться на бесплатных планах
- ❌ Может "засыпать" через 10-15 минут

**Как включить webhook:**
Просто установите переменную `WEBHOOK_DOMAIN` в Railway!
...
### Бот "засыпает" и не отвечает на сообщения

**Симптомы:** Бот работает после деплоя, но через 10-15 минут перестаёт отвечать.

**Причина:** Используется polling режим вместо webhook.

**Решение:**
1. Проверьте режим работы:
   ```bash
   curl https://ваш-домен.railway.app/health
   ```
   Должно быть `"mode":"webhook"`

2. Если показывает `"mode":"polling"`, добавьте переменную в Railway:
   ```
...
4. Проверьте логи - должна быть строка:
   ...
   ✅ Бот успешно запущен в режиме webhook!
   ```

**Почему webhook лучше:**
- Telegram мгновенно отправляет обновления на ваш сервер
